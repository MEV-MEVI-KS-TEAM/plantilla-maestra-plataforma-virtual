/**
 * El normalizador de las ofertas de Cursos de Ingreso, SIN imports.
 *
 * Existe aparte de oferta.ts (que lee CONFIG) porque también lo usa el generador
 * de la entrega (scripts/entrega/cursos-entrega.mjs), que corre en Node con el
 * type stripping nativo y no entiende los alias '@/'. Así la entrega revisa las
 * MISMAS ofertas que muestra el registro. oferta.ts re-exporta todo lo de aquí.
 */
...
export interface OfertaIngreso {
   ...
   * curso cuando la tiene (ver resolverPrecioOferta en precio-regla.ts). En el
