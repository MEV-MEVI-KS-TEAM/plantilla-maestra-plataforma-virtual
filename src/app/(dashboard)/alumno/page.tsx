...
import {
  GraduationCap, Bell, CheckCircle2, ChevronRight, Mail,
} from 'lucide-react'
...
export default function AlumnoDashboard() {
  ...
  // WhatsApp es editable desde "Personalizar mi página"; `pagos` no, y sigue
  // leyendo CONFIG.
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      {demo && (
        <div className="rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4"
          ...
          {/* El canal que la escuela SÍ tiene: WhatsApp, o correo si no hay
              número. Sin ninguno no hay botón (antes: `wa.me/` vacío). */}
          {canal && (
            <a href={canal.href} target="_blank" rel="noopener noreferrer"
              className={`flex items-center gap-2 ${canal.tipo === 'whatsapp' ? 'bg-green-500 hover:bg-green-600' : ''} text-white px-4 py-3 rounded-xl font-semibold transition-colors whitespace-nowrap flex-shrink-0`}
              style={canal.tipo === 'whatsapp' ? undefined : { background: 'var(--color-acento)' }}>
              {canal.tipo === 'whatsapp' ? (
              ...
              {canal.tipo === 'whatsapp' ? 'Contactar por WhatsApp' : canal.etiqueta}
            </a>
          )}
        </div>
      )}
      ...
      {!demo && !perfil.inscripcion_pagada && (
        <div className="rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4"
          ...
          {CONFIG.pagos?.activo ? (
            ...
            <a href={canal.href} target="_blank" rel="noopener noreferrer"
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap flex-shrink-0 ${canal.tipo === 'whatsapp' ? 'bg-green-500 hover:bg-green-600' : ''} transition-colors`}
              style={canal.tipo === 'whatsapp' ? { color: '#fff' } : { color: '#fff', background: 'var(--color-acento)' }}>
              {canal.tipo === 'whatsapp' ? (
              ...
              {canal.tipo === 'whatsapp' ? 'WhatsApp' : canal.etiqueta}
            </a>
